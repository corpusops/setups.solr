# docker build --squash -t corpusops/solr:5.4.0 . -f Dockerfile --build-arg=SKIP_COPS_UPDATE=y --build-arg=COPS_IMG=corpusops/solr:5.4.0
#FROM corpusops/solr
...
RUN bash -c '\
  ...
  && rsync -av /srv/projects/solr/data/ /srv/projects/solr/data.sample/'
